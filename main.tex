...
\end{eqnarray}
\endgroup
The term $f^{c(v)}_{w}$ represents the carrier occupancy of the QW. Likewise, $f^{c(v)}_{u,j},f^{c(v)}_{e,j},f^{c(v)}_{g,j}$ represent the carrier occupancy of the $j$th group of US, ES, and GS respectively. The term $\Gamma_{k}$ is the optical confinement factor at wavelength $\lambda_{k}$. The terms $P_{k,in}$ and $P_{p}$ represent the power of the $k$th photon mode and probe signal, respectively. The term $\alpha(\lambda_{(k,p)})$ represents the loss coefficient at the wavelength $\lambda_{(k,p)}$, which is the sum of the scattering and absorption losses. The term $g_{mod}(t,\lambda_{(k,p)})$ is the effect on linear modal gain caused by slow light, and it can be expressed as the product of the slowdown factor, optical-confinement factor, and linear material gain of the active region.
% ÇÊÇËè⁄ÇµÇ≠
