...
All-optical AND gate using photonic crystal(PC) quantum-dot semiconductor optical amplifiers(QDSOA) is presented in this paper. We numerically analyze the input-output characteristics of the gate by using rate equation model and show the gate can operate at 160Gb/s. Moreover, in order to show the effectiveness of the proposed gate, we compare the proposed gate with conventional one in terms of degree of integration, energy consumption and performance.}
...
% 研究の意義・これからやることの説明
Few researches discuss about the length of device which is important factor for developing high-density integrated and less energy consumption device. 
% 具体的な内容を追加
Thus, we propose PC-QDSOA assisted all-optical AND gate. AND gate plays an important role for multiplexer and demultiplexer circuits whose applications are communication system, computer memory and arithmetic logic unit. In order to show the effectiveness of the proposed gate, we numerically analyze the input-output characteristics of the gate by using rate equation model. To our knowledge, there is no research about numerically analyzing PC-QDSOA assisted all-optical AND gate. The results in this paper show that the realization of the all-optical AND gate is feasible with the proposed PC-QDSOA assisted all-optical AND gate at 160Gb/s and the proposed gate can achieve 9dB extinction ratio(ER) at most. Furthermore, we show the required current injection for all-optical AND gate can be reduced with using the proposed gate.

\section{PC-QDSOA model}
% PC-QDSOAの構成について
% Fig.1の説明を追加
The schematic diagram of PC-QDSOA waveguide is illustrated in Fig.{\ref{fig:pcqdsoa}}. The PC-QDSOAs used in this study are composed of GaAs, ${\rm{In_{0.15}Ga_{0.85}As}}$ and InAs as an active region to enhance optical power around ${\rm{1.3\mu m}}${\cite{theory_of_qdsoa}}. The population inversion in the active regions of amplifiers is achieved by passing current through laterally doped ${\rm{p^{+}-p-n^{+}}}$ structures. Also, the waveguide of the PC-QDSOA is W1 PCW wherein slow light can be achieved with either zero group velocity dispersion and zero third order dispersion in specific bandwidth. The waveguide can be achieved by creating a line defect in a slab PC, which can be obtained by creating round vacancy periodically on n-cladding and p-cladding regions{\cite{how_to_create_pcw}}. 
