...
% TODO:ìKêÿÇ»ï\åªÇ…ïœÇ¶ÇÈ
\abstract{An all-optical AND gate using photonic-crystal quantum dot semiconductor optical amplifiers is designed and its performance is evaluated. The input-output characteristics of the gate are simulated using rate equation model and it is found that the gate can achieve a maximum of approximately 9-dB extinction ratio at 160 Gb/s using the parameters shown in this paper. The proposed gate is compared with quantum dot semiconductor optical amplifiers AND gate to evaluate its effectiveness with regard to signal output, device size, and power consumption.}
